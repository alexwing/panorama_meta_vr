Shader "Unlit/Pano360ShaderURP3D"
{
    Properties
    {
        _MainTexLeft ("Base Left Eye (RGB)", 2D) = "white" {}
        _MainTexRight ("Base Right Eye (RGB)", 2D) = "white" {}
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                if (unity_StereoEyeIndex == 0)
                    col = tex2D(_MainTexLeft, uv);
                else
                    col = tex2D(_MainTexRight, uv);
